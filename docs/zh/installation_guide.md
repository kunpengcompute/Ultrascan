...
- 增加HAO算法引擎，作为基于并行位提取的多模匹配加速算法，通过编译期构建哈希过滤表和运行期并行位提取降低无效候选和后端校验开销，提升鲲鹏平台匹配吞吐量。
...
|SQLite|SQLite 3|可选：Ultrascan tools测试工具hsbench编译依赖SQLite 3。|使用Yum工具安装。|
...
Ultrascan tools工具hsbench编译依赖SQLite 3，使用Yum命令安装SQLite及SQLite开发套件，安装完成后进行版本验证。
