...
public:
    virtual ~Generator() {}
    virtual void configure(const std::string &type, int depth, int count,
                           bool fullCharset) = 0;
    virtual std::vector<FuzzTestCase> generate() = 0;
    virtual size_t
    generateTo(const std::function<bool(const FuzzTestCase &)> &consumer) {
        size_t delivered = 0;
...
public:
    virtual ~Runner() {}
    virtual bool compile(const FuzzTestCase &testCase,
                         unsigned int mode = HS_MODE_BLOCK) = 0;
    ...
    virtual bool
    compileExtMulti(const std::vector<FuzzTestCase> &testCases) = 0;
    virtual bool compileLit(const FuzzTestCase &testCase, size_t length) = 0;
    virtual bool compileLitMulti(const std::vector<FuzzTestCase> &testCases,
                                 const std::vector<size_t> &lengths) = 0;
    virtual bool fatCompile(const FuzzTestCase &testCase,
                            unsigned int mode = HS_MODE_BLOCK) = 0;
    virtual bool fatCompileMulti(const std::vector<FuzzTestCase> &testCases,
                                 unsigned int mode = HS_MODE_BLOCK) = 0;
    virtual bool fatCompileExtMulti(const std::vector<FuzzTestCase> &testCases,
                                    unsigned int mode = HS_MODE_BLOCK) = 0;
    virtual bool fatCompileLit(const FuzzTestCase &testCase, size_t length,
                               unsigned int mode = HS_MODE_BLOCK) = 0;
    virtual bool fatCompileLitMulti(const std::vector<FuzzTestCase> &testCases,
    ...
    virtual bool
    falsePositiveFeedback(const FuzzTestCase &testCase,
